  ...
  try {
        ...
        if (!calendarsResponse.ok) {
          ...
          return new Response(
            JSON.stringify({ 
              ...
              requiresConnection: calendarsResponse.status === 401
            }),
            { 
              headers: { ...corsHeaders, 'Content-Type': 'application/json' },
              status: 200 
    ...
    try {
      ...
      if (calendarsResponse.ok) {
        ...
        console.error('Failed to fetch calendars for sync:', calendarsResponse.status);
        if (calendarsResponse.status === 401) {
          return new Response(
            JSON.stringify({ 
              error: 'Google Calendar authentication expired. Please reconnect your account.',
              success: false,
              requiresConnection: true
            }),
            { 
